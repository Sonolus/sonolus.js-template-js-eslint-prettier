...
const preprocessComponent = (index) => {
    index -= 1
...
const exitCurrentSegment = (index) => {
    if (current === next) return
...
const updateCurrentSegment = (index) => {
...
const forEach = (items, callback) => items.map(() => callback)
